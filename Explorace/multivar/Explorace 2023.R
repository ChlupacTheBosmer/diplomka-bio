...
nrow(combined[`Any contact`==TRUE & `include in network analyses`==TRUE,])
...
points(rdout, disp="sites",pch=zygomorph+16,cex=1.3, col = "black")
...
ordispider(rdout, openess, display = "sites", spiders = c("median"), col = openess+3)

barplot(as.numeric(eigenvals(rdout) / sum(eigenvals(rdout))))

#Klasifikace na základě traits
dis <- vegdist(towork) #bray-curtisova vzdalenost je default
clus <- hclust(dis, "average")
plot(clus,hang=0.5)
rect.hclust(clus, 6)
grp <- cutree(clus, 6)
ordispider(rdout, grp, display = "sites", spiders = c("median"), col = grp+3)
##Klasifikace na základě návštěvníků
dis <- vegdist(websp[[1]]) #bray-curtisova vzdalenost je default
clus <- hclust(dis, "average")
plot(clus,hang=0.5)
rect.hclust(clus, 8)
grp <- cutree(clus, 8)
boxplot(Traits_and_indices_df[elev==1100 & dry==1,"Average of Final size"]~grp[which(names(grp) %in% Traits_and_indices_df[elev==1100 & dry==1,"SPCODE"])])
points(towork$length~grp, pch=zygomorph+16,cex=1.0, col = rgb(TaI$R/255,TaI$G/255,TaI$B/255))
...
TS$dry <- TaI$dry
towork <- TS[,!colnames(TS) %in% c("elev","dry")]
towork <- towork[,!colnames(towork) %in% c("R","G","B","bright", "sugar", "zygomorph", "odour", "guides", "hue")]
#towork <- towork[,colnames(towork) %in% c("R","G","B","bright", "sugar", "odour","guides")]
par(mfrow=c(1,1))
...
rdout = rda(towork~Tstand$d+fE+fD)
rdout = rda(towork~fE+fD)
...
#test vlivu ůrovně specializace (lisi se květní traits s úrovní specializace polinaního systému?)
...
rdout=rda(towork~Tstand$d+Condition(fE+fD))  
CTRL <- how(nperm = 500, blocks=fE, plots=Plots(strata=fD,type="none"),within=Within(type="free"))
CTRL
anova(rdout,by="terms",permutations=CTRL) 
